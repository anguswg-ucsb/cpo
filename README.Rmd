...
Now that we have identified the mainstems within our AOI, and the most upstream flowlines of these mainstems, we can use these upstream points to identify the water rights we want to select for each district. First we'll get all the relevant water rights for the district, and remove any missing data, removing data whose `water_source` is "GROUNDWATER", and applying a spatial filter to just points within 500 meters of a mainstem river.
...
After subsetting our data, we have 802 unique water rights, which correspond to 236 unique WDID locations
```{r, eval = TRUE, echo = TRUE}
...
We then identify the indices of the points nearest to most upstream flowline, and thus we've found the most upstream WDIDs along our mainstem flowlines. 
```{r, eval = TRUE, echo = TRUE}
...
The gif below walks through the process described above for finding the most upstream WDIDs for each of our mainstem flowlines in the district of interest:

```{r, eval = FALSE, echo = FALSE}

sub_flines <- dplyr::mutate(  
...
pts2 <- pts1 %>% sf::st_filter(aoi) 

pts3 <- dplyr::filter(pts2, !grepl("GROUNDWATER", water_source))
pts4 <- sf::st_filter(
  pts3,
  sf::st_transform(
      sf::st_buffer(
        sf::st_union(full_net), 500),
      4326)
  )
# pts_match <- find_closest_matches(toupper(aoi$name),  unique(pts3$water_source))[[1]]
# all(max_pts$gnis_id %in% pts2$gnis_id)
# pts4 <- dplyr::filter(pts3, water_source == pts_match | gnis_id %in% max_pts$gnis_id)
...
wr_pts_plot00 <- 
  ...
  # ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
    ggplot2::geom_sf(data =  sub_flines, ggplot2::aes(color = streamleve)) +
    gghighlight::gghighlight(comid %in% "00000000000") +
    # ggplot2::geom_sf(data = pts1) +
    # ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
  ggplot2::labs(
    # title = "Colorado water districts by basin",
    fill = "",
...
ggplot2::ggsave(
  wr_pts_plot00,
  filename = "img/00_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
ggplot2::ggsave(
  wr_pts_plot0,
  filename = "img/01_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
# ggplot2::ggsave(
#   wr_pts_plot1,
#   filename = "img/02_water_rights.png",
#   width    = 12,
#   height   = 9,
#   scale    = 1
#   )
...
ggplot2::ggsave(
  wr_pts_plot3,
  filename = "img/03_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
wr_fline_plot1 <-
  ...
  # ggplot2::geom_sf(data = pts) +
  # ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
    ggplot2::geom_sf(data =  sub_flines, ggplot2::aes(color = terminalpa)) +
    gghighlight::gghighlight(comid %in% full_net$comid) +
...
ggplot2::ggsave(
  wr_fline_plot1,
  filename = "img/04_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
wr_fline_plot2 <-
  ...
  # ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
    ggplot2::geom_sf(data =  sub_flines, ggplot2::aes(color = streamleve)) +
    gghighlight::gghighlight(comid %in% "00000000000000000000") +
...
ggplot2::ggsave(
  wr_fline_plot2,
  filename = "img/05_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
ggplot2::ggsave(
  wr_fline_plot3,
  filename = "img/06_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
ggplot2::ggsave(
  wr_fline_plot4,
  filename = "img/07_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
# fline_pts <- sf::st_filter(  pts3, 
#   sf::st_transform(sf::st_buffer(full_net, 3000), 4326)
#   )
# wr_pts_plot3 <-
#   ggplot2::ggplot() +
#   ggplot2::geom_sf(data = aoi) +
...
near1 <- sf::st_nearest_feature(
  sf::st_transform(upstream_fline[1, ], 4326),
  pts4
  # pts3
  )

near2 <- sf::st_nearest_feature(
  sf::st_transform(upstream_fline[2, ], 4326),
  pts4
    )
# pts[c(near1, near2), ]

final_pts <- sf::st_buffer(pts4[c(near1, near2), ], 250) 
...
wr_pts_plot4 <-
  ...
  # ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
    ...
    ggplot2::geom_sf(data = pts4) +
    # ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
  ggplot2::geom_sf(data = final_pts[1, ], fill = "#00ba38") +
      # ggplot2::geom_sf(data = sf::st_buffer(pts[c(near1, near2), ], 250), fill = "#00ba38") +
  ggplot2::labs(
    # title = "Colorado water districts by basin",
    fill = "",
    caption = "( 9 / 12 )"
...
ggplot2::ggsave(
  wr_pts_plot4,
  filename = "img/08_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
wr_pts_plot5 <-
  ...
  # ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
    ...
    ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +  
    ggplot2::geom_sf(data = pts4) +
    ggplot2::geom_sf(data = final_pts, fill = "#00ba38") +
      # ggplot2::geom_sf(data = sf::st_buffer(pts[c(near1, near2), ], 250), fill = "#00ba38") +
  ggplot2::labs(
    # title = "Colorado water districts by basin",
    fill = "",
...
ggplot2::ggsave(
  wr_pts_plot5,
  filename = "img/09_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
wr_pts_plot6 <-
  ...
  # ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
    ...
    gghighlight::gghighlight(comid %in% "00000000000") +
    ggplot2::geom_sf(data = sf::st_buffer(max_pts, 100), fill = "red") +
  ggplot2::geom_sf(data = final_pts, 
                   fill = "#00ba38"
                   ) +
      # ggplot2::geom_sf(data = sf::st_buffer(pts[c(near1, near2), ], 250), fill = "#00ba38") +
  ggplot2::labs(
...
ggplot2::ggsave(
  wr_pts_plot6,
  filename = "img/10_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )
...
wr_pts_plot7 <-
  ...
  ggplot2::geom_sf(
    # data = final_pts, 
    data =  sf::st_buffer(final_pts, 200),
                   fill = "#00ba38"
                   ) +
      # ggplot2::geom_sf(data = sf::st_buffer(pts[c(near1, near2), ], 250), fill = "#00ba38") +
  ggplot2::labs(
...
ggplot2::ggsave(
  wr_pts_plot7,
  filename = "img/11_water_rights.png",
  width    = 12,
  height   = 9,
  scale    = 1
  )

# ---- MAKE WATER RIGHTS GIF ----
# library(gifski)
wr_pngs <- sort(list.files("img", pattern = "_water_rights", full.names = T))
gifski::gifski(wr_pngs,
                gif_file = "gif/find_water_rights.gif",
                width = 2000,
                height =1600,
                delay = 1.70,
                loop = TRUE,
...
![Workflow for finding upstream water rights on mainstem rivers in an AOI](gif/find_water_rights.gif)
...
Here we use `cdssr` to get call analysis data for each of WDIDs of interest 
...
The call anaylsis by WDID tool from CDSS, allows us to gather a daily timeseries for each of the WDIDs we selected in each district. 

The percent out of priotity timeseries provides 2 key data points that we used for our analysis:
1. Daily out of priority percentage
2. The water right date of the downstream water right that calls out junior upstream users.

For each daily timestamp, we wanted to determine the most senior right that would call out of super junior upstream WDID of interest for each mainstem in each district. For each of our WDIDs that we collected call out data for, we calculated an annual average call year which indicates for a given mainstem river and a given year, what the average water right call year was for that mainstem/year.
