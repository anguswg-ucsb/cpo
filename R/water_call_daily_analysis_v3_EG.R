...
water_call_analysis <- function(df_call, df_predictor, distr){
    ...
    write_csv(call_detrended_data, "data/detrended_call_data.csv")
...
model <- function(detrended_all_data_final, distr){
    rm(list = ls())

    detrended_all_data_final <- detrended_all_data_final[detrended_all_data_final$district %in% distr,]
    # test linear regression models
    lm1 <- lm(call_year_decimal~may_swe, detrended_all_data_final)
    lm2 <- lm(call_year_decimal~may_fx_apr_to_sep, detrended_all_data_final)
...
df_predictor <- df_predictor[df_predictor$district %in% c(1,2,3,4,5,6,7,8,9,23,64,80),]
...
detrended_all_data_final <- water_call_analysis(df_call, df_predictor, distr)

# Specify district(s) for linear regression and plotting:
distr <- c(80)# 1, 2, 3, 4, 5, 6, 7, 8, 9, 23, 64, 80)

linear_regression <- model(detrended_all_data_final, distr)



#################################################################################################
# Part 5: Plotting/Visualizations
# NEED TO FIX SO THAT CALL_DETRENDED_DATA IS ALSO AN OUTPUT OF THE FUNCTION...
#
...
p1 <- ggplot(detrended_all_data_final, aes_string(x="year", y = "may_swe", group = "wdid")) +
  #stat_summary(fun.data=mean_cl_normal) +
...
p2 <- ggplot(detrended_all_data_final, aes_string(x="year", y = "may_fx_apr_to_sep", group = "wdid")) +
  #stat_summary(fun.data=mean_cl_normal) +
  geom_line(aes(col = as.factor(wdid), linetype = as.factor(wdid))) +
  #geom_smooth(method='lm', formula= y~x) +
  theme_bw() +
...
p3 <- ggplot(detrended_all_data_final, aes_string(x="year", y = "call_year_decimal", group = "wdid")) +
  #stat_summary(fun.data=mean_cl_normal) +
  geom_line(aes(col = as.factor(wdid), linetype = as.factor(wdid))) +
  #geom_smooth(method='lm', formula= y~x) +
  theme_bw() +
...
p6 <- ggplot(detrended_all_data_final, aes(x = may_swe, y = call_year_decimal)) +
  stat_summary(fun.data=mean_cl_normal) +
...
p7 <- ggplot(detrended_all_data_final, aes(x = may_fx_apr_to_sep, y = call_year_decimal)) +
  stat_summary(fun.data=mean_cl_normal) +
  geom_point(aes(color = as.factor(wdid))) +
  geom_smooth(method='lm', formula= y~x) +
  theme_bw() +
